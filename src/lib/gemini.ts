...
import { consumeAiQuestion, FREE_QUESTION_LIMIT } from '@/src/lib/subscription';
...
class FirebaseAIChat {
  ...
  async sendMessage({
    ...
    try {
      const entitlement = await consumeAiQuestion(auth.currentUser.uid);
      ...
      if (!entitlement.isPro && entitlement.questionsUsed >= FREE_QUESTION_LIMIT) {
      ...
      if (/TRIAL_LIMIT_REACHED/.test(raw)) {
        showTrialLimit();
