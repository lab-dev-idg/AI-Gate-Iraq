...
import { CheckCircle2, Crown, Loader2, Mail, X } from 'lucide-react';
...
export default function TrialLimitDialog() {
  ...
  const [requestState, setRequestState] = useState<'idle' | 'sent' | 'pending' | 'active' | 'error'>('idle');
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/80 px-4 py-8 backdrop-blur-md" role="dialog" aria-modal="true" dir={lang === 'en' ? 'ltr' : 'rtl'}>
      <div className="relative w-full max-w-lg rounded-[2rem] border border-amber-300/20 bg-[#101827] p-7 text-white shadow-2xl sm:p-9">
        ...
        {feedback ? (
          <div className={`mt-6 flex items-start gap-2 rounded-2xl border px-4 py-3 text-xs font-bold leading-6 ${requestState === 'error' ? 'border-rose-400/20 bg-rose-400/10 text-rose-200' : 'border-emerald-400/20 bg-emerald-400/10 text-emerald-200'}`}>
            {requestState !== 'error' ? <CheckCircle2 className="mt-1 h-4 w-4 shrink-0" /> : null}
            <span>{feedback}</span>
          </div>
        ) : null}
