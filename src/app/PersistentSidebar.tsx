...
export default function PersistentSidebar({ activeService, setActiveService, lang }: PersistentSidebarProps) {
  ...
  const startNewChat = () => {
    const session = loadSession(lang as any);
  ...
  const restoreConversation = (conversation: SavedConversation) => {
    saveSession({ activeService: 'assistant', chatScope: conversation.service, chatMessages: conversation.messages });
    window.location.reload();
  };
    ...
    >
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex h-14 shrink-0 items-center justify-center border-b border-slate-200 px-2 dark:border-slate-800 lg:justify-between lg:px-3">
          ...
          <button type="button" onClick={toggleSidebar} aria-label={sidebarToggleLabel} title={sidebarToggleLabel} className="group flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-blue-400/30 bg-blue-600 text-white shadow-lg shadow-blue-600/20 transition-all hover:-translate-y-0.5 hover:bg-blue-500 hover:shadow-blue-600/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-white active:translate-y-0 dark:ring-offset-[#0B1220]">
            {expanded ? <PanelLeftClose className="h-5 w-5 transition-transform group-hover:scale-110" /> : <PanelLeftOpen className="h-5 w-5 transition-transform group-hover:scale-110" />}
          </button>
        </div>
