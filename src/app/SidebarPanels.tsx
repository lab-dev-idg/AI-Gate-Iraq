...
export function SidebarSearchPanel({ ku, onRestore }: { ku: boolean; onRestore: (conversation: SavedConversation) => void }) {
    ...
    <div className="flex min-h-0 flex-1 flex-col p-3">
      <label className="relative block shrink-0">
        ...
        <input
          ...
          placeholder={ku ? 'گەڕان لە چاتەکان...' : 'البحث في المحادثات...'}
      ...
      <div className="no-scrollbar mt-3 min-h-0 flex-1 space-y-1 overflow-y-auto">
        {filtered.length === 0 ? (
          <p className="p-6 text-center text-sm text-slate-500 dark:text-slate-400">{ku ? 'هیچ چاتێک نەدۆزرایەوە.' : 'لم يتم العثور على محادثات.'}</p>
          ...
          <div key={conversation.id} className="group flex items-center gap-2 rounded-xl p-2 hover:bg-slate-100 dark:hover:bg-slate-800">
            ...
            <button type="button" onClick={() => { deleteConversation(conversation.id); setConversations(listConversations()); }} className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 opacity-0 hover:text-rose-500 group-hover:opacity-100">
...
export function SidebarProjectsPanel({ ku, activeService, onOpen }: { ku: boolean; activeService: ServiceKey; onOpen: (service: ServiceKey) => void }) {
  return (
    <div className="no-scrollbar min-h-0 flex-1 overflow-y-auto p-3">
      <h3 className="mb-3 text-xs font-black text-slate-500 dark:text-slate-400">{ku ? 'پڕۆژە و شوێنەکانی کار' : 'المشاريع ومساحات العمل'}</h3>
      <div className="space-y-1">
        {SERVICES.map((service) => {
          const Icon = service.icon;
          const active = service.key === activeService;
          return (
            <button key={service.key} type="button" onClick={() => onOpen(service.key)} className={`flex h-12 w-full items-center gap-3 rounded-xl px-3 text-right ${active ? 'bg-[#1F6FEB] text-white' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800'}`}>
              <Icon className={`h-5 w-5 shrink-0 ${active ? 'text-white' : service.color}`} />
              <span className="min-w-0 flex-1 truncate text-sm font-bold">{ku ? service.label_ku : service.label_ar}</span>
            </button>
          );
        })}
      </div>
