...
export const ChatMessages = ({
  ...
  // Chat scroll lock: no visible white scrollbar rail on mobile.
  return (
    <div 
      ref={chatScrollRef} 
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg, idx) => (
            <motion.div
              ...
              className={`flex w-full max-w-full min-w-0 ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}
            >
              <div className={`flex gap-3 max-w-[92%] sm:max-w-[86%] lg:max-w-[78%] ${msg.role === 'user' ? 'flex-row' : 'flex-row-reverse'} w-full min-w-0`}>
