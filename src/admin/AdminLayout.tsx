    ...
    <div className="min-h-screen text-slate-100 bg-[#070b13] flex flex-col font-sans selection:bg-emerald-500/20" dir="rtl">
      <header className="sticky top-0 z-40 w-full bg-[#0a0f1d]/95 border-b border-slate-800/80 backdrop-blur-md px-4 md:px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden md:inline-block text-xs font-medium text-slate-400 font-sans">کۆنترۆڵی گشتی پلاتفۆرم</span>
        </div>

        <div className="flex items-center gap-2">
          <Button onClick={onBackToApp} variant="ghost" size="sm" className="h-9 px-3 rounded-xl border border-slate-800 hover:bg-slate-800 text-slate-300 hover:text-white text-xs gap-1.5 font-bold transition-all">
            <ArrowLeft className="w-4 h-4" />
            <span className="hidden sm:inline">گەڕانەوە بۆ پلاتفۆرم</span>
          </Button>

          <Button onClick={onLogout} variant="ghost" size="sm" className="h-9 w-9 sm:w-auto sm:px-3 rounded-xl hover:bg-rose-500/10 hover:text-rose-400 text-slate-400 text-xs gap-1.5 font-bold transition-all">
      ...
      <div className="flex-1 w-full flex items-stretch relative min-h-0">
        <aside className="hidden lg:block w-64 border-l border-slate-800/80 bg-[#090e19] shrink-0 p-4 space-y-2 select-none">
          <div className="px-3 pb-2 mb-3 text-[10px] font-black uppercase tracking-widest text-slate-500">ناونیشانی بەشەکان</div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-start gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${isActive ? 'bg-emerald-600 font-extrabold text-white shadow-lg shadow-emerald-600/10' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'}`}
                >
          ...
          <div className="lg:hidden fixed inset-0 z-30 flex">
            ...
            <div className="relative flex-1 max-w-[280px] bg-[#090e19] p-4 space-y-4 flex flex-col min-h-full mr-auto select-none border-l border-slate-800">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">بژاردەکان</span>
              ...
              <nav className="flex-1 space-y-1">
                {navItems.map((item) => {
                  ...
                  return (
                    <button
                      key={item.key}
                      onClick={() => { onSectionChange(item.key); setIsMobileMenuOpen(false); }}
                      className={`w-full flex items-center justify-start gap-3 px-3.5 py-3 rounded-xl text-xs font-bold transition-all ${isActive ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800'}`}
                    >
              ...
              <div className="pt-2 border-t border-slate-800/80">
                <Button onClick={onBackToApp} variant="outline" className="w-full h-10 border-slate-800 text-slate-300 text-xs font-bold mb-2 gap-1.5">
                  <ArrowLeft className="w-4 h-4" /><span>گەڕانەوە بۆ پلاتفۆرم</span>
                </Button>
                <Button onClick={onLogout} variant="ghost" className="w-full h-10 hover:bg-rose-500/10 hover:text-rose-400 text-slate-400 text-xs gap-1.5">
                  <LogOut className="w-4 h-4" /><span>چوونە دەرەوە</span>
                </Button>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 min-w-0 flex flex-col bg-[#070b13] overflow-y-auto">
          <div className="p-4 md:p-6 lg:p-8 max-w-6xl w-full mx-auto space-y-6">{children}</div>
        </main>
      </div>
    </div>
  );
