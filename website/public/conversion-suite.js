(() => {
  ...
  const ensureForm = () => {
    ...
    host.innerHTML = `<div class="container"><div class="conversion-shell"><aside><span class="section-kicker">AI Gate Iraq</span><h2>${t('title')}</h2><p>${t('lead')}</p><ol><li class="active">${t('step1')}</li><li>${t('step2')}</li><li>${t('step3')}</li></ol></aside><form id="conversionForm" novalidate><input id="conversionWebsite" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true" style="position:absolute;left:-9999px"><div class="conversion-step active" data-step="0"><div class="form-grid"><label for="conversionCompany">${t('company')}<input id="conversionCompany" name="company" autocomplete="organization" required maxlength="120"></label><label for="conversionName">${t('name')}<input id="conversionName" name="name" autocomplete="name" required maxlength="100"></label><label for="conversionEmail">${t('email')}<input id="conversionEmail" name="email" autocomplete="email" type="email" required maxlength="160"></label><label for="conversionPhone">${t('phone')}<input id="conversionPhone" name="phone" autocomplete="tel" inputmode="tel" required minlength="6" maxlength="32"></label><label for="conversionSector">${t('sector')}<input id="conversionSector" name="sector" autocomplete="organization-title" required maxlength="100"></label><label for="conversionTeam">${t('team')}<select id="conversionTeam" name="team" required><option value=""></option><option>1-10</option><option>11-50</option><option>51-200</option><option>200+</option></select></label></div></div><div class="conversion-step" data-step="1"><div class="form-grid"><label for="conversionIntent">${t('intent')}<select id="conversionIntent" name="intent" required><option value=""></option><option value="demo">${t('demo')}</option><option value="pilot">${t('pilot')}</option><option value="partner">${t('partner')}</option></select></label><label for="conversionTimeline">${t('timeline')}<select id="conversionTimeline" name="timeline" required><option value=""></option><option>0-30 days</option><option>1-3 months</option><option>3-6 months</option><option>6+ months</option></select></label><label class="full" for="conversionChallenge">${t('challenge')}<textarea id="conversionChallenge" name="challenge" required minlength="20" maxlength="1200" rows="7"></textarea></label></div></div><div class="conversion-step" data-step="2"><div id="conversionReview" class="conversion-review"></div><label class="consent-row"><input id="conversionPrivacyConsent" type="checkbox" name="privacyConsent" required><span>${t('consent')}</span></label></div><div class="form-status" id="conversionStatus" role="status" aria-live="polite"></div><div class="form-actions"><button type="button" class="button button-secondary" id="conversionBack">${t('back')}</button><button type="button" class="button button-primary" id="conversionNext">${t('next')}</button><button type="submit" class="button button-primary" id="conversionSubmit">${t('submit')}</button></div></form></div></div>`;
