(() => {
  ...
  const copy = {
    ku: { title:'داوای پیشاندانی تایبەت یان تاقیکردنەوەی سەرەتایی', lead:'زانیارییە سەرەکییەکان بنووسە؛ تیمەکەمان پێداچوونەوە دەکات و ڕێڕەوی گونجاو پێشکەش دەکات.', step1:'زانیاری کۆمپانیا', step2:'پێویستی و ئامانج', step3:'پێداچوونەوە و ناردن', next:'هەنگاوی دواتر', back:'گەڕانەوە', submit:'ناردنی داواکاری', company:'ناوی کۆمپانیا', name:'ناوی بەرپرسی پەیوەندی', email:'ئیمەیڵی کاری', phone:'ژمارەی پەیوەندی', sector:'بواری کار', team:'قەبارەی تیم', intent:'جۆری داواکاری', demo:'پیشاندانی تایبەت', pilot:'تاقیکردنەوەی سەرەتایی', partner:'هاوبەشی ستراتیژی', challenge:'گرنگترین کێشە یان پێویستی', timeline:'کاتی دەستپێکردنی پێشبینیکراو', consent:'ڕەزامەندم زانیارییەکانم بۆ وەڵامدانەوە بە داواکارییەکەم بەکاربهێنرێن.', success:'داواکارییەکەت ئامادە کرا. بەرنامەی ئیمەیڵەکەت دەکرێتەوە بۆ ناردن.', cookieTitle:'ڕێکخستنی تایبەتمەندی', cookieText:'کوکییە پێویستەکان بۆ کارکردنی وێبسایت بەکاردێن. شیکارییە هەڵبژاردەییەکان تەنها بە ڕەزامەندی تۆ چالاک دەبن.', accept:'پەسەندکردنی هەموو', reject:'تەنها پێویستەکان', settings:'ڕێکخستن', save:'پاشەکەوتکردن', analytics:'شیکاری و پێوانە', analyticsText:'یارمەتیمان دەدات تێبگەین کام بەشەکان بەکاردێن، بەبێ کۆکردنەوەی زانیاری هەستیار.', privacy:'کۆنتڕۆڵی تایبەتمەندی' },
    ar: { title:'طلب عرض خاص أو تجربة أولية', lead:'أدخل المعلومات الأساسية ليقوم فريقنا بمراجعتها واقتراح المسار المناسب.', step1:'بيانات الشركة', step2:'الحاجة والهدف', step3:'المراجعة والإرسال', next:'التالي', back:'رجوع', submit:'إرسال الطلب', company:'اسم الشركة', name:'اسم جهة الاتصال', email:'البريد المهني', phone:'رقم الاتصال', sector:'قطاع العمل', team:'حجم الفريق', intent:'نوع الطلب', demo:'عرض خاص', pilot:'تجربة أولية', partner:'شراكة استراتيجية', challenge:'أهم تحد أو احتياج', timeline:'موعد البدء المتوقع', consent:'أوافق على استخدام بياناتي للرد على طلبي.', success:'تم تجهيز طلبك. سيفتح تطبيق البريد لإرساله.', cookieTitle:'إعدادات الخصوصية', cookieText:'نستخدم ملفات أساسية لتشغيل الموقع، ولا نفعل التحليلات الاختيارية دون موافقتك.', accept:'قبول الكل', reject:'الأساسية فقط', settings:'الإعدادات', save:'حفظ', analytics:'التحليلات', analyticsText:'تساعدنا على فهم استخدام الأقسام دون جمع بيانات حساسة.', privacy:'ضوابط الخصوصية' },
    en: { title:'Request a Private Demo or Initial Pilot', lead:'Provide the core information and our team will review it and propose the appropriate path.', step1:'Company Details', step2:'Need and Objective', step3:'Review and Submit', next:'Next', back:'Back', submit:'Submit Request', company:'Company Name', name:'Contact Name', email:'Business Email', phone:'Phone', sector:'Sector', team:'Team Size', intent:'Request Type', demo:'Private Demo', pilot:'Initial Pilot', partner:'Strategic Partnership', challenge:'Primary challenge or need', timeline:'Expected Start', consent:'I consent to the use of my information to respond to this request.', success:'Your request is ready. Your email application will open to send it.', cookieTitle:'Privacy Settings', cookieText:'Essential storage is used to operate the site. Optional analytics is enabled only with your consent.', accept:'Accept All', reject:'Essential Only', settings:'Settings', save:'Save', analytics:'Analytics', analyticsText:'Helps us understand section usage without collecting sensitive information.', privacy:'Privacy Controls' }
  };
  ...
  const ensureForm = () => {
    ...
    form.addEventListener('submit', event => {
      ...
      const data = Object.fromEntries(new FormData(form).entries());
      const subject = encodeURIComponent(`AI Gate Iraq — ${data.intent} request from ${data.company}`);
      const body = encodeURIComponent(Object.entries(data).filter(([key]) => key !== 'privacyConsent').map(([key,value]) => `${key}: ${value}`).join('\n'));
      status.textContent = t('success');
      track('conversion_submit', { intent: data.intent, sector: data.sector });
      try { localStorage.removeItem(STORAGE.draft); } catch {}
      location.href = `mailto:contact@aigateiraq.com?subject=${subject}&body=${body}`;
    });
    render();
  };
